...
namespace DiasGames.ThirdPersonSystem.ClimbingSystem
{
    public class WallClimb : ThirdPersonAbility
    {
        ...
        [SerializeField] private float m_HeightToClimbUp = 1.8f;
        ...
        public override bool TryEnterAbility()
        {
            ...
            {
                ResetCollider();
                if(!m_InputManager.jumpButton.IsPressed)
                    return false;
            }
        ...
        {
            return (m_System.IsGrounded && m_InputManager.Move.y < 0) || (m_InputManager.dropButton.IsPressed && !m_System.IsCoroutinePlaying);
        }
        ...
        {
            Vector3 Start = transform.position + Vector3.up * (m_HeightToClimbUp + 0.5f);
            RaycastHit hit;
        ...
        {
            Vector3 StartPointTop = transform.position + Vector3.up * m_HeightToClimbUp - transform.forward * m_System.m_Capsule.radius;
