...
namespace DiasGames.ThirdPersonSystem
{
    ...
    public abstract class ThirdPersonAbility : MonoBehaviour
    {
        ...
        [Tooltip("Name of enter state of ability")]
        [SerializeField]
        protected string m_EnterState = "";

        [Tooltip("Animation transition duration to play this animation")]
        [SerializeField]
        protected float m_TransitionDuration = 0.1f;

        [Tooltip("Multiplier of root motion velocity in all axis")]
        [SerializeField]
        protected Vector3 m_RootMotionMultiplier = Vector3.one;

        [Tooltip("Should exit this ability when animation ends?")]
        [SerializeField]
        protected bool m_FinishOnAnimationEnd = false;

        [Space(5)]
        // -----------------------------------------------------------------------------------------------------------------------------------//

        [Tooltip(
            "Abilities that must be ignored to enter this ability. Means that this ability has higher priority than these ignored abilities"
        )]
        public List<ThirdPersonAbility> IgnoreAbilities;

        [Tooltip("Should this ability uses root motion?")]
        [SerializeField]
        protected bool m_UseRootMotion = true;

        [Tooltip("Should this ability uses rotation root motion?")]
        [SerializeField]
        protected bool m_UseRotationRootMotion = false;

        [Tooltip("Should this ability uses root motion in vertical direction?")]
        [SerializeField]
        protected bool m_UseVerticalRootMotion = false;

        [SerializeField]
        protected bool m_AllowCameraZoom = false; // Allow camera zoom?

        [SerializeField]
        protected PhysicMaterial m_AbilityPhysicMaterial;
        ...
        {
            get { return m_UseRotationRootMotion; }
        }
        public bool UseVerticalRootMotion
        {
            get { return m_UseVerticalRootMotion; }
        }
        public Vector3 RootMotionMultiplier
        {
            get { return m_RootMotionMultiplier; }
        }

        public bool AllowCameraZoom
        {
            get { return m_AllowCameraZoom; }
        }
        ...
        public PhysicMaterial AbilityPhysicMaterial
        {
            get { return m_AbilityPhysicMaterial; }
            set { m_AbilityPhysicMaterial = value; }
        }
        ...
        public UnityEvent OnEnterAbilityEvent,
            OnExitAbilityEvent;

        protected string m_CurrentStatePlaying = ""; // Get current state playing
        public string CurrentStatePlaying
        {
            get { return m_CurrentStatePlaying; }
        }

        protected InputButton m_InputToEnter = null;

        [SerializeField]
        protected InputEnterType m_UseInputStateToEnter = InputEnterType.Noone;

        [SerializeField]
        protected InputReference InputButton;
        ...
        protected virtual void FixedUpdate()
        {
            ...
            if (Active)
            {
                ...
                if (
                    m_FinishOnAnimationEnd
                    && m_AnimatorManager.HasFinishedAnimation(m_CurrentStatePlaying)
                )
                {
        ...
        public virtual bool TryEnterAbility()
        {
            return false;
        }
        ...
        /// </summary>
        /// <returns>True: enter ability</returns>
        public virtual bool ForceEnterAbility()
        {
            return false;
        }
        ...
        public virtual bool TryExitAbility()
        {
            return false;
        }
        ...
        public virtual string GetEnterState()
        {
            return m_EnterState;
        }
        ...
        protected virtual void SetState(string newState, float transitionDuration)
        {
            ...
            m_AnimatorManager.SetAnimatorState(
                newState,
                transitionDuration,
                AnimatorManager.BaseLayerIndex
            );
        ...
        protected bool FreeOnMove()
        {
            ...
            Vector3 p2 =
                transform.position
                + Vector3.up * (m_System.m_Capsule.height - m_System.m_Capsule.radius);

            RaycastHit[] hits = Physics.CapsuleCastAll(
                p1,
                p2,
                m_System.m_Capsule.radius * 0.5f,
                transform.forward,
                m_System.m_Rigidbody.velocity.sqrMagnitude * Time.fixedDeltaTime + 0.25f,
                m_System.GroundMask,
        ...
        protected bool FreeOnMove(Vector3 direction)
        {
            ...
            {
                if (
                    hit.normal.y <= Mathf.Cos(m_System.MaxAngleSlope * Mathf.Deg2Rad)
