...
namespace DiasGames.ThirdPersonSystem
{
    public class JumpAbility : ThirdPersonAbility
    {
        ...
        [SerializeField]
        private float m_MaxJumpHeight = 1.0f;

        [Tooltip("The maximum horizontal speed that character can have during the jump")]
        [SerializeField]
        private float m_MaxHorSpeed = 5f;

        [Tooltip("State of stationary jump")]
        [SerializeField]
        private string m_StationaryJumpState = "Air.JumpInPlace";

        private float VerticalSpeed
        {
            get { return Mathf.Sqrt(-2 * Physics.gravity.y * m_MaxJumpHeight); }
        }
        ...
        {
            base.Update();
            if (m_System.IsGrounded && exit && m_System.m_Rigidbody.velocity.y < 0)
            {
                jumpCount = maxJumpCount;
                m_System.m_Animator.SetFloat("DoubleJump", 0);
            }
            if (
                jumpCount > 0
                && Input.GetKeyDown(KeyCode.Space)
                && !m_System.IsGrounded
                && (ThirdPersonSystem.free || jumpCount < maxJumpCount)
            )
            {
                if (!Mathf.Approximately(m_InputManager.RelativeInput.magnitude, 0))
                {
                    m_System.m_Animator.SetFloat("DoubleJump", 1);
                    OnEnterAbility();
                }
                else if (!m_System.IsGrounded)
                {
                    m_UseVerticalRootMotion = false;
                    m_System.m_Animator.SetFloat("DoubleJump", 1);
                    m_System.m_Rigidbody.AddForce(
                        new Vector3(0, VerticalSpeed * 10000, 0) * Time.deltaTime
                    );
                    OnEnterAbility();
                }
            }
