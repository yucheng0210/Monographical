...
//	This seams to be calculated in a 2nd evaluation? and thus operates on the extruded vertices
	float colorVariation = saturate ( ( frac(TreeWorldPos.x + TreeWorldPos.y*0 + TreeWorldPos.z) + frac( (TreeWorldPos.x + TreeWorldPos.y*0 + TreeWorldPos.z) * 3.3 ) ) * 0.5 );
vsColorVariation = colorVariation;
...
// 	Here we need the billboardNormal in absolute WS
	//float3 viewVecAbsWS = normalize(_WorldSpaceCameraPos - ( worldPos) );
	float3 viewVecAbsWS = normalize(unity_BillboardCameraPosition - (worldPos) );
...
// 	We have to flip everthing as we are rendering backfaces?!
	normalOut = normalize( -billboardNormal.xyz );
	tangentOut = billboardTangent.xyz * float3(-1,1,-1);
