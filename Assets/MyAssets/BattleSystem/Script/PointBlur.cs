...
public class PointBlur : MonoBehaviour
{
    ...
    private void OnRenderImage(RenderTexture source, RenderTexture destination)
    {
        RenderTexture rt1 = RenderTexture.GetTemporary(
            source.width >> downSampleFactor,
            source.height >> downSampleFactor,
            0,
            source.format
        );
    ...
    private void Update()
    {
        ...
        material.SetVector(
            "_BlurCenter",
            new Vector4(BlurCenter.x * Camera.main.aspect, BlurCenter.y, 0, 0)
        );
        ...
        if (Input.GetMouseButtonDown(0))
        {
            ...
            if (Physics.Raycast(ray, out hit))
            {
                ...
                BlurCenter = Camera.main.WorldToScreenPoint(
